...
//
//static uint16_t line_buf[LCD_WIDTH * LCD_HEIGHT];
static uint16_t line_buf[LCD_WIDTH * LCD_HEIGHT];
...
// Draw the entire screen divided into 4 quadrants
void
st7789_draw_quad_screen(uint16_t tl_color, uint16_t tr_color, uint16_t bl_color, uint16_t br_color)
{
  for(int row = 0; row < LCD_HEIGHT; row++)
  {
    for(int col = 0; col < LCD_WIDTH; col++)
    {
      int idx = row * LCD_WIDTH + col;
      ...
      if(col < LCD_WIDTH /2 )
      {
        ...
        if(row < LCD_HEIGHT / 2)
        {
          line_buf[idx] = tl_color;  // top-left
        }
        else
        {
          line_buf[idx] = bl_color;  // bottom-left
        }
      }
      else
      {
        ...
        if(row < LCD_HEIGHT / 2)
        {
          line_buf[idx] = tr_color;  // top-right
        }
        else
        {
          line_buf[idx] = br_color;  // bottom-right
        }
      }
    }
  }
